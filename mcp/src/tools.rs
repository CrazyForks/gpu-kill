...
impl ToolHandler {
    ...
    async fn create_user_policy(
        ...
        if let Some(guard_mode) = &mut self.guard_mode {
            let memory_limit = args
                ...
                .and_then(|v| v.as_f64())
                .unwrap_or(8.0);
            let utilization_limit = args
                .get("utilization_limit_pct")
                .and_then(|v| v.as_f64())
                .unwrap_or(70.0);
            let process_limit = args
                .get("process_limit")
                .and_then(|v| v.as_u64())
                .unwrap_or(3) as u32;
    ...
    async fn kill_processes_by_name(
        &mut self,
        arguments: Option<HashMap<String, serde_json::Value>>,
    ) -> anyhow::Result<ToolResult> {
        let args = arguments.ok_or_else(|| anyhow::anyhow!("Missing arguments"))?;
                ...
                if let Some(ref mut pm) = self.process_manager {
                    ...
                    Ok(ToolResult {
                        content: vec![ToolContent {
                            content_type: "text".to_string(),
                            text: Some(
                                "Process management not available on this system".to_string(),
                            ),
                            data: None,
                        }],
                        is_error: Some(true),
